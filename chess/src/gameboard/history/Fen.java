...
public class Fen {
    ...
    public ArrayList<Spot> interpret(String fen, Position position) {
        ...
        position.setEnpassant(
            (info[3].equals("-")) ? 0 : (Board.algebraicToSquare(info[3]))
        );
        return pieces;
    }
    ...
    public String createFen(Player[] players, Position position) {
        ...
        if (gameBoard.getCurrentPositon().getEnpassant() != 0) {
            result += Board.SQUARE_TO_ALGEBRAIC[gameBoard.getCurrentPositon().getEnpassant()];
        } else {
            result += "-";
        }
        // Pawn counters
    ...
    /**
     * @param ch char from fen
     * @return true if char is number between (0-8, including 0 and 8),
     * false otherwise
     */
